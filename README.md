...
- WordAnalysis note: the WordAnalysis layer is used for word selection, curriculum gating, and safe substitutions only. Child-facing reading text and choice buttons render as plain text; phonics markings are not displayed.
- Character note: the player sprite keeps the same pixel shape, but skin tone now uses the `SKIN_TONES` presets in `main.js`. The default is a neutral medium palette, and the in-game Settings button cycles tones and saves the choice in local storage.
- Gameplay UI note: during play, repeated header/sidebar guide text is minimized so the board, the current prompt, and the reading modal carry the action with less distraction.
- Word selection note: Block Quest now cross-checks its legacy selector against a stricter timing-based word engine. The `Selection Audit` panel (open with `?debugWords=1`) shows tiers, scores, repeats, and heart-word timing, while child-facing text still renders as plain unmarked words.
- Runtime engine bundle note: advanced word-selection runtime files now live in `/Users/joshbarocas/Documents/block-quest/runtime-word-engine` (`engine-runtime-v1.js`, `engine-data-v1.js/json`, `engine-manifest.json`). `index.html` loads these before `main.js` for offline `file://` use.
- Curriculum options note: program select now supports `Fundations`, `UFLI Foundations`, and `From Phonics to Reading` (`program=from_phonics_to_reading` in URL params).
- Verification note: open `index.html?debugWords=1&program=from_phonics_to_reading&unit=8` to inspect old-vs-new selection diffs, tiering, concept blocking/support, mapping path, and engine version.
- Engine update checklist:
  1) Export fresh runtime data from `word_engine_codex_repo_v2` into `runtime-word-engine/engine-data-v1.js` and `runtime-word-engine/engine-data-v1.json`.
  2) Keep `runtime-word-engine/engine-runtime-v1.js` and `runtime-word-engine/engine-manifest.json` in sync (version/checksums/compat schema).
  3) Open `index.html?debugWords=1&program=fundations&unit=13`, `index.html?debugWords=1&program=ufli&unit=80`, and `index.html?debugWords=1&program=from_phonics_to_reading&unit=15` and confirm old-vs-new diffs render.
  4) If runtime config/schema fails validation, Block Quest should fall back to legacy selection and show a warning in the Selection Audit summary.
- Fundations Level 2 note: selector timing now includes units 11–14 concepts (diphthongs, suffixes/prefixes, morphology, multisyllable/schwa tags). This pass uses a conservative mapping in the engine and can be tightened further with exact extracted L2 unit wording.
- Phoneme inventory note: the selector now uses a broader grapheme inventory (44-phoneme-informed) to adjust analysis confidence, so words with recognized grapheme chunks are favored while ambiguous chunks are down-weighted.
